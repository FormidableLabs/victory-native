...
const VictoryVoronoiHelpers = {
  ...
  getDatasets(props) {
    ...
    return VictoryHelpers.reduceChildren(React.Children.toArray(props.children), iteratee);
  },
...
const Helpers = {
  ...VictoryVoronoiHelpers,
  onTouchMove(evt, targetProps) { // eslint-disable-line max-statements
    const activePoints = targetProps.activePoints || [];
    const mousePosition = NativeHelpers.getSVGEventCoordinates(evt);
    ...
    if (activePoints.length && isEqual(points, activePoints)) {
      // same points are active
      return parentMutations;
    } else {
      // different points are active
