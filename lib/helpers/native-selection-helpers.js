import { Selection, Data, Helpers as VictoryHelpers } from "victory-core";
import { assign, throttle, isFunction } from "lodash";
import React from "react";
import { NativeHelpers } from "../index";

const VictorySelectionHelpers = {
  getDatasets(props) {
    if (props.data) {
...
const Helpers = {
  ...VictorySelectionHelpers,
  onTouchStart(evt, targetProps) {
    const { dimension, scale } = targetProps;
    const datasets = targetProps.datasets || [];
    const {x, y} = NativeHelpers.getSVGEventCoordinates(evt);
